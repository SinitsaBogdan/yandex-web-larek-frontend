...
import { SuccessUI } from './components/success/views/SuccessUI';
...
import { ProductCatalogUI } from './components/product/views/ProductCatalogUI';
import { IProductType } from './components/product/types/IProductType';
import { ProductPreviewUI } from './components/product/views/ProductPreviewUI';
import { BasketShortUI } from './components/basket/view/BasketShortUI';
import { BasketUI } from './components/basket/view/BasketUI';
import { ProductBasketUI } from './components/product/views/ProductBasketUI';
import { IContactsData, IPaymentData } from './components/order/types/IOrderType';
import { PaymentUI } from './components/order/views/PaymentUI';
import { ContactsUI } from './components/order/views/ContactsUI';
import { PageUI } from './components/page/PageUI';
import { IPresenters } from './components/app/types/IPresenters';
import { IComponents } from './components/app/types/IComponents';
...
const app = new ApplicationsModel<IPresenters, IComponents>(
	{
	...
	},
	{
		...
		basketFull: new BasketUI(cloneTemplate(basketFullTemplate), events, CONST.BASKET_SETTINGS),
...
events.on(EVENT.API_CATALOG_GET_ALL, () => {
	...
	api.catalog.getProducts().then((data) => {
		data.forEach((item) => {
			const product: IProductType = new ProductModel(item, events);
...
events.on(EVENT.API_CATALOG_GET_ID, (data) => {
...
events.on(
	...
	({ field, value }: { field: keyof IPaymentData | keyof IContactsData; value: string }) => {
...
events.on(EVENT.FORM_PAYMENT_ERRORS, ({ valid, message }: { valid?: boolean; message?: string }) => {
	...
	else app.components.payment.render({ valid: false, errors: message, next: EVENT.RENDER_ORDER_CONTACTS });
});
...
events.on(EVENT.FORM_CONTACTS_ERRORS, ({ valid, message }: { valid?: boolean; message?: string }) => {
	...
	else app.components.contacts.render({ valid: false, errors: message, next: EVENT.API_ORDER_POST });
});
